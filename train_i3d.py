...
def run(configs,
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=configs.batch_size, shuffle=False, num_workers=0,
                                             pin_memory=True)
    ...
    if mode == 'flow':
        ...
        i3d.load_state_dict(torch.load('weights/asl100/FINAL_nslt_100_iters=896_top1=65.89_top5=84.11_top10=89.92.pt', map_location=torch.device('cpu')))
    else:
        ...
        # for phase in ['test']:
            ...
            for data in dataloaders[phase]:
                ...
                for i in range(per_frame_logits.shape[0]):
                    print("gt[i] item: ", torch.argmax(gt[i]).item())
                    print("predictions[i] item: ", torch.argmax(predictions[i]).item())
                ...
                if num_iter == num_steps_per_update // 2:
                    print("Epoch: ", epoch, " Steps: " ,steps, " loss.data.item(): ", loss.data.item())
                loss.backward()
                ...
                if num_iter == num_steps_per_update and phase == 'train':
                    ...
                    # lr_sched.step()
                    if steps % 10 == 0:
                        acc = float(np.sum(np.diag(confusion_matrix)) / np.sum(confusion_matrix))
                        print(
            ...
            if phase == 'test':
                val_score = float(np.sum(np.diag(confusion_matrix)) / np.sum(confusion_matrix))
...
if __name__ == '__main__':
    # WLASL setting
    mode = 'rgb'
    root = {'word': '/Users/tabaxitft/Desktop/WLASL/data/WLASL2000'}

    save_model = 'checkpoints/'
    train_split = 'preprocess/nslt_100.json'

    # weights = 'archived/asl2000/FINAL_nslt_2000_iters=5104_top1=32.48_top5=57.31_top10=66.31.pt'
    # weights = 'weights/asl100/FINAL_nslt_100_iters=896_top1=65.89_top5=84.11_top10=89.92.pt'
    weights = None
    config_file = 'weights/asl100/FINAL_nslt_100_iters=896_top1=65.89_top5=84.11_top10=89.92.ini'
